...
class Dates extends React.Component { 
  ...
  getDates = ()=>{
    const {date, dates,  month, year, selectedDate} = this.props;
          console.log(this.state.events[selectedDate+"."+month+"."+year] + " props")
    if (date.getDay() == 0) {
    ...
    while (String(date.getMonth()) == month) {
    ...
    Object.entries(dates).map(([day, event])=>{
      if(selectedDate == event[0] && this.state.events[selectedDate+"."+month+"."+year]){
  ...
  render(){
    return (<>{Object.entries(this.getDates()).sort((a, b) => a[0] - b[0]).map(([day, event], i)=>{
        return(
        <div className={(event[0] == this.props.todayD && 
                        this.props.todayMonth == this.props.month &&
                        this.props.todayYear == this.props.year )?("calender__date today"):("calender__date")}
              key={this.props.month + day + String(this.props.date.getDay()) + i}
